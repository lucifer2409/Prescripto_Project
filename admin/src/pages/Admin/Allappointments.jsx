  ...
  return (
    <div className='w-full max-w-6xl m-5'>
      ...
      <div className='bg-white border rounded text-sm max-h-[80vh] overflow-y-scroll min-h-[60vh]'>
        <div className='hidden sm:grid grid-cols-[0.5fr_3fr_1fr_3fr_3fr_1fr_1fr] grid-flow-col py-3 px-5 border-b'>
          <p>#</p>
          <p>Patient</p>
          <p>Age</p>
          <p>Date & Time</p>
        ...
        {appointments.map((item , index)=> (
          <div key={index} className='flex flex-wrap justify-between max-sm:gap-2 sm:grid-cols-[0.5fr_3fr_1fr_3fr_3fr_1fr_1fr] items-center text-gray-500 py-3 px-6 border-b hover:bg-gray-50'>
            ...
            <div className='flex items-center gap-2'>
              <img src={item.userData.image} alt="patientsimage" className='w-8 rounded-full'/><p>{item.userData.name}</p>
            </div>
            <p className='max-sm:hidden'>{calculateAge(item.userData.dob)}</p>
            <p>{slotDateFormat(item.slotDate)} , {item.slotTime}</p>
            <div className='flex items-center gap-2'>
              <img src={item.docData.image} alt="patientsimage" className='w-8 rounded-full bg-gray-200'/><p>{item.docData.name}</p>
            </div>
